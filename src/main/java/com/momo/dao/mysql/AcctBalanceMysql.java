...
@Repository
public class AcctBalanceMysql implements AcctBalanceDao {
    //注入JdbcTemplate
    @Autowired
    ...
    public Map<String, String> getAcctBalanceByAcctId(Long acctId) {
        String sql = "select * from acct_balance where acct_id = ?";
    ...
    public Integer deleteByAcctId(Long acctId) {
        String sql = "delete from acct_balance where acct_id = ?";
